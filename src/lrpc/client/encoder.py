...
def _check_bytearray(value: LrpcType, var: LrpcVar) -> memoryview:
...
def _check_string(value: LrpcType, var: LrpcVar) -> str:
...
def _encode_bytearray(ba: memoryview) -> bytes:
    return struct.pack("<B", ba.nbytes) + ba.tobytes()


def _encode_string(s: str, var: LrpcVar) -> bytes:
...
def _encode_optional(value: LrpcType | None, var: LrpcVar, lrpc_def: LrpcDef) -> bytes:
    if value is None:
...
def _check_struct(value: LrpcType, var: LrpcVar, lrpc_def: LrpcDef) -> dict[str, LrpcType]:
...
def _encode_struct(value: dict[str, LrpcType], var: LrpcVar, lrpc_def: LrpcDef) -> bytes:
    encoded = b""
    s = lrpc_def.struct(var.base_type())
...
def _check_array(value: LrpcType, var: LrpcVar) -> list[LrpcType]:
...
def _encode_array(value: list[LrpcType], var: LrpcVar, lrpc_def: LrpcDef) -> bytes:
    encoded = b""
...
def _check_enum_field_id(value: LrpcType, var: LrpcVar, lrpc_def: LrpcDef) -> str:
    if not isinstance(value, str):
...
def _encode_basic_type(pack_type: str, value: LrpcBasicType) -> bytes:
...
def lrpc_encode(value: LrpcType, var: LrpcVar, lrpc_def: LrpcDef) -> bytes:
    ...
    if var.is_optional():
        return _encode_optional(value, var, lrpc_def)

    if var.base_type_is_bytearray():
        value = _check_bytearray(value, var)
        return _encode_bytearray(value)

    if var.base_type_is_string():
        value = _check_string(value, var)
        return _encode_string(value, var)

    if var.base_type_is_struct():
        value = _check_struct(value, var, lrpc_def)
        return _encode_struct(value, var, lrpc_def)

    if var.base_type_is_enum():
        value = _check_enum_field_id(value, var, lrpc_def)
        e = lrpc_def.enum(var.base_type())
        field_id = e.field_id(value)
        if field_id is None:
            raise ValueError(f"{value} is not a field in enum {var.name()}")
        return _encode_basic_type(var.pack_type(), field_id)

    if not isinstance(value, (bool, int, float, str)):
        raise TypeError(f"Type error for {var.name()}: expected bool, int, float or str, but got {type(value)}")

    return _encode_basic_type(var.pack_type(), value)
