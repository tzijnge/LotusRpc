...
|          | [functions](#functions)                                     |
|          | [streams](#streams)                                         |
|          | [functions_before_streams](#functions-and-streams-ordering) |

`name` is the name of the service. It must be a valid C++ identifier.
[`id`](#service-id) is the service ID. When not explicitly specified, it is generated by LotusRPC.
...
`name` is the name of the struct. It must be a valid C++ identifier. `fields` is a list of data members, every member being a [LrpcType](#lrpctype). Custom structs can be referenced inside the LotusRPC definition file by prepending the name with the `@` sign.
