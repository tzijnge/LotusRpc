...
The meta service has a [service ID](reference#service-id) of 255. It is not possible to use this ID for any user services.
...
| Field             | Type   | Content                                                                                                                                                                                                                    |
|-------------------|--------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `definition`      | string | User-supplied version string from [`settings.version`](reference_settings#version). Empty string if not set.                                                                                                               |
| `definition_hash` | string | SHA3-256 of the rendered YAML definition file (UTF-8), as a 64-character hex string. Optionally truncated — see [`definition_hash_length`](reference_settings#definition_hash_length). Empty string if length is set to 0. |
...
If the server has [`embed_definition: true`](reference_settings#embed_definition) set, `lrpcc` can retrieve the definition directly from the server at startup, eliminating the possibility of a client/server definition drift:

```yaml
definition_from_server: always   # always fetch from server; definition_url is ignored
```

See [`definition_from_server`](tools#lrpcc) in the tools reference for the `once` and `never` modes and their trade-offs.

### Definition

The `definition` stream (finite, origin: server) in the meta service returns the embedded definition in chunks as big as possible with the current [tx_buffer_size](reference_settings#rx_buffer_size--tx_buffer_size) setting. Each chunk is of type [bytearray](reference#lrpctype).

For more info on embedding the definition in the server, see the [settings reference](reference_settings#embed_definition).
