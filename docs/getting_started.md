...
Include the file `<out-dir>/example/battery_shim.hpp` in your project. Derive you own service class from `ex::battery_shim` and implement all pure virtual functions. These are the remote procedures that are called when issuing a function call on the client. Implement these functions as desired.
